...
public class ProjectionPlanNode extends AbstractPlanNode {
    ...
    public void generateOutputSchema(Database db) {
        ...
        NodeSchema newSchema = new NodeSchema(columns.size());
        for (SchemaColumn col : columns) {
            AbstractExpression colExpr = col.getExpression();
            SchemaColumn colCopy;
            if (colExpr.getExpressionType().isAggregateExpression()) {
                if (null == inputSchema.find(col.getTableName(),
                        col.getTableAlias(),
                        col.getColumnName(),
                        col.getColumnAlias())) {
                ...
                colCopy = col.copyAndReplaceWithTVE(colIndex);
            }
            else {
                colCopy = col.clone();
            }
            newSchema.addColumn(colCopy);
            ++colIndex;
        }
        m_outputSchema = newSchema;
        m_hasSignificantOutputSchema = true;
