...
//            }
        ...
        else if (parsedStmt instanceof ParsedSwapStmt) {
            if (tableListIncludesExportOnly(parsedStmt.m_tableList)) {
     ...
     *
     * @return The next (only) for a given SWAP TABLE statement, then null.
     */
    private CompiledPlan getNextSwapPlan() {
        ...
        Table targetTable0 = m_parsedSwap.m_tableList.get(0);
        Table targetTable1 = m_parsedSwap.m_tableList.get(1);
        CompiledPlan retval = new CompiledPlan();
        retval.setReadOnly(false);

        // the root of the SWAP TABLE plan is always a SwapPlanNode
        SwapTablesPlanNode swapNode = new SwapTablesPlanNode();
        swapNode.init(targetTable0, targetTable1);
        ...
        // Add a compensating sum of modified tuple counts or a limit 1
        // AND a send on top of the union-like receive node.
        boolean isReplicated = targetTable0.getIsreplicated();
     ...
     *
     * @return The next (only) for a given insert statement, then null.
     */
