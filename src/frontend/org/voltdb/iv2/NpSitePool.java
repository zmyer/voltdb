...
/**
 * Provide a pool of NP transactions.
 */
class NpSitePool {
    ...
    private List<NpSiteContext> m_allSites = Collections.synchronizedList(new ArrayList<>());
            ...
            InitiatorMailbox initiatorMailbox)
    {
        ...
        Integer poolSize = Integer.getInteger("npiPoolSize");
        if (poolSize == null) {
            poolSize = DEFAULT_MAX_POOL_SIZE;
        }
        m_poolSize = poolSize;
        tmLog.info("Setting maximum size of NP site pool to: " + m_poolSize);
